...
# Run the pipeline (cached in session_state by filter combination)
def _make_cache_key(filters_used, k):
    """Build a hashable key from filters + k for session caching."""
    import hashlib, json
    raw = json.dumps({"filters": filters_used, "k": k}, sort_keys=True, default=str)
...
cache_key = _make_cache_key(filters_used, k)
if st.session_state.get("_profiler_cache_key") == cache_key:
    # Same filters + k → reuse cached results (instant)
    assignments = st.session_state["_profiler_assignments"]
    metrics = st.session_state["_profiler_metrics"]
    summary = st.session_state["_profiler_summary"]
    profiles = st.session_state["_profiler_profiles"]
else:
    # New filters or k → run clustering
    ...
    # Cache for next rerun
    st.session_state["_profiler_cache_key"] = cache_key
    st.session_state["_profiler_assignments"] = assignments
    st.session_state["_profiler_metrics"] = metrics
    st.session_state["_profiler_summary"] = summary
    st.session_state["_profiler_profiles"] = profiles
...
for container, profile in zip(containers, profiles):
    with container:
        ...
        # ── Save Pattern ──────────────────────────────────────────────
        _pattern_name = profile_names.get(cid, f"Pattern {cid}")

        # Check if already saved
        _existing_patterns = db.get_all_patterns()
        _combo_for_save = {
            "grgr_ck": filters_used.get("grgr_ck"),
            "sgsg_ck": filters_used.get("sgsg_ck"),
            "cspd_cat": filters_used.get("cspd_cat"),
            "lobd_id": filters_used.get("lobd_id"),
            "grgr_name": _grgr,
            "sgsg_name": _sgsg,
            "cspd_cat_desc": _cat,
            "plds_desc": _lob,
        }
        _pattern_key = db._make_pattern_key(_combo_for_save, cid)
        _already_saved = any(
            p.get("pattern_key") == _pattern_key and p.get("saved_by")
            for p in _existing_patterns
        )

        if _already_saved:
            st.success(f"✓ Pattern saved: **{_pattern_name}**")
        else:
            if st.button("⭐ Save Pattern", key=f"save_pattern_{cid}", type="secondary"):
                _profile_for_save = {
                    k: profile[k] for k in
                    ("cluster_id", "size", "pct_of_subset", "continuous",
                     "categorical", "family", "descriptions")
                ...
                db.save_pattern_from_profiler(
                    combo=_combo_for_save,
                    cluster_id=cid,
                    contextual_name=_pattern_name,
                    member_count=profile["size"],
                    pct_of_pop=profile.get("pct_of_subset", 0),
                    silhouette=metrics.get("silhouette", 0),
                    profile=_profile_for_save,
                )
                st.success(f"Pattern **{_pattern_name}** saved! View it in Saved Patterns.")
                st.rerun()
...
# Build short legend labels — strip the common hierarchy prefix shared by all patterns
_all_names = [profile_names.get(cid, f"P{cid}") for cid in unique_clusters]
if len(_all_names) > 1:
    # Find common prefix parts
    _split_names = [n.split("-") for n in _all_names]
    _min_parts = min(len(s) for s in _split_names)
    _common_len = 0
    for i in range(_min_parts):
        if len(set(s[i] for s in _split_names)) == 1:
            _common_len = i + 1
        else:
            break
    # Keep only the distinguishing suffix
    legend_names = {
        cid: "-".join(name.split("-")[_common_len:]) or f"P{cid}"
        for cid, name in zip(unique_clusters, _all_names)
    }
else:
    legend_names = {cid: name for cid, name in zip(unique_clusters, _all_names)}
    ...
    for i, cid in enumerate(unique_clusters):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name=legend_names.get(cid, f"P{cid}"),
                legendgroup=f"c{cid}",
            ),
            row=1, col=1,
        )
